...
local osc_styles = {
	...
	Tooltip = "{\\blur1\\bord0.5\\1c&HFFFFFF&\\3c&H000000&\\fs"
		...
		.. user_opts.font
		.. "}",
	Title = "{\\blur1\\bord0.5\\1c&HFFFFFF&\\3c&H0\\fs"
								...
								if dur > 0 then
									...
									if ch and ch.title and ch.title ~= "" then
										...
										elem_ass:pos(
											...
											thumbY * r_h - user_opts.timefontsize
										)
										elem_ass:an(an)
...
function checktitle()
	...
	local artist = mp.get_property("filtered-metadata/by-key/Artist")
		or mp.get_property("filtered-metadata/by-key/Album_Artist")
...
function window_controls()
	...
	local first_geo = { x = controlbox_left + 30, y = button_y, an = 5, w = 40, h = wc_geo.h }
	local second_geo = { x = controlbox_left + 74, y = button_y, an = 5, w = 40, h = wc_geo.h }
	local third_geo = { x = controlbox_left + 118, y = button_y, an = 5, w = 40, h = wc_geo.h }
...
layouts = function()
	...
	geo = {
		...
		y = refY
			- 122
			+ (((state.localDescription ~= nil or state.isWebVideo) and user_opts.showdescription) and -20 or 0),
		an = 1,
		w = osc_geo.w - 50,
		h = 35,
	}
	...
	if showinfo then
		...
		lo.geometry = {
			x = osc_geo.w - 172 + (showloop and 0 or 50) + (showontop and 0 or 50),
			y = refY - 40,
			an = 5,
			w = 24,
			h = 24,
	...
	if user_opts.downloadbutton then
		...
		lo.geometry = {
			x = osc_geo.w - 217 + (showloop and 0 or 50) + (showontop and 0 or 50) + (showinfo and 0 or 50),
			y = refY - 40,
			an = 5,
			w = 24,
			h = 24,
...
function osc_init()
	...
	ne.visible = (
		osc_param.playresx
		>= 800 - outeroffset - (user_opts.showloop and 0 or 100) - (user_opts.showontop and 0 or 100)
	)
	ne.eventresponder["mbtn_left_up"] = function()
	...
	ne.eventresponder["mouse_move"] = function(element)
		...
		if (element.state.lastseek == nil) or not (element.state.lastseek == setvol) then
...
function adjustSubtitles(visible)
	if
		visible
		and user_opts.raisesubswithosc
		and state.osc_visible == true
		and (state.fullscreen == false or user_opts.showfullscreen)
	then
		local w, h = mp.get_osd_size()
		if h > 0 then
